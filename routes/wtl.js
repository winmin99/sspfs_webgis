...
export default {
  ...
  search(req, res, next) {
    postgresql.executeQuery(
      `SELECT *
       FROM view_search
       WHERE (fac_nam LIKE $1 OR fac_typ LIKE $1 OR fac_uid = $2)         
       ORDER BY fac_typ, (CASE WHEN fac_nam IS NULL OR fac_nam = '' THEN 1 ELSE 0 END), fac_nam ASC;`,
      [`%${req.query['query']}%`, req.query['query']], // TODO: '상수' Role 처리
    ).then(result => {
