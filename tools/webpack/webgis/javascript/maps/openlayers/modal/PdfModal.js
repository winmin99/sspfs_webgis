...
function onClickOverlay(event) {
  const newFeature = selectInteraction.getFeatures().getArray().map(f => {
    ...
    const pdfname = `관리대장_${number}.pdf`
    return pdfname
  })
...
// Create modal content
  ...
  modalContent.innerHTML = `<div class="navbar-nav">
            <div class="nav-item text-nowrap" id="pdfmodal">
              <a href="pdfjs-4.0.189-dist/web/viewer.html?file=${newFeature}">크게보기</a>
              <br>
              <iframe src="pdfjs-4.0.189-dist/web/viewer.html?file=${newFeature}" style="width:100%; height:300px; border:1px solid black;"></iframe>
            </div>
          </div>
...
// Attach open and close modal functions to elements
  ...
  // Attach close modal function to the modal container
